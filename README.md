...
* Optionally constrain drag interactions with an indexed geometry constraint that keeps robot pair distances no larger than the corresponding sheet vertex distances while preserving convexity and vertex order.
...
* Run `VvcmFk` in the browser and display candidate solution counts, stability labels, object poses, virtual object points, taut cable indices, matching lambda values, and a stable-only FK result filter.
* Visualize the sheet polygon, robot formation, cable reference lines, single-solution taut `vi-vo` and `ri-ro` segments, and selected or filtered-all FK solution positions with matching solution colors in the canvas and FK result list.
...
The header includes GitHub links to the VVCM source repository and the VVCM Web source repository.

Use the edit mode control and point tables to choose whether you are editing sheet vertices or robot positions; the canvas emphasizes the active edit layer so robots stand forward while editing robot positions and sheet vertices stand forward while editing the sheet. Use the sheet view and robot view checkboxes in the Coordinate Canvas header to show or hide those canvas layers; the sheet view includes `vo` markers, and the robot view includes `po` markers. Enable the geometry constraint checkbox to restrict drag interactions with the indexed condition `||r_i-r_j|| <= ||v_i-v_j||`, convex polygon shape, and matching sheet/robot vertex order. Robot vertex drags show same-radius guide circles centered on the other robot vertices and clamp to the feasible disk intersection; sheet vertex drags show forbidden guide circles centered on other sheet vertices and clamp outside those circles. Constrained drags keep a small safety margin inside the feasible region so FK receives points that are not exactly on a numerical boundary. The constraint is drag-only, so table edits, JSON edits, and pasted full configurations can still create invalid geometry; when the checkbox is enabled, the app highlights and reports any existing violations without automatically moving points. Drag a point marker or its label to move that point. Drag empty canvas space to pan the view, use the mouse wheel or the `+` and `-` controls to zoom, and use `Fit` to reframe the currently visible sheet, robot, and FK result geometry.

The 3D view uses the same VVCM coordinates directly with Z as the vertical axis. The legend describes robot ground points as `r# ground`, elevated hold points as `p# hold point`, visible FK object points as `po object`, and taut links as `taut p#-po`; in-scene point labels use the active index base, such as `r0`/`p0` or `r1`/`p1`. Orbit, pan, and zoom the 3D view with the mouse or touchpad, and use `Reset` to fit the current scene. Switching the shown FK solution preserves the current 3D camera view. 3D labels scale with the view to stay readable, and camera movement stops immediately when input ends. In single-solution mode the selected solution's taut cables are drawn from the relevant `p#` hold point to `po`; in all-solutions mode every currently visible `po` is shown with that solution's taut `p#-po` links.
...
The FK result panel reads every candidate branch from `solutions.solutions`, marks each branch as stable or unstable, shows each branch's `po`, `vo`, taut cable indices, and taut-only lambda values, and lets you show one branch or all currently visible branches on the canvas. The stable-only filter is enabled by default, so the list, selector, canvas, and 3D view initially show stable branches only; disable it to inspect unstable branches too. When exactly one branch is visible, taut cables from that branch are highlighted as `vi-vo` and `ri-ro` segments. In all-branches mode, object and virtual-object markers for every branch allowed by the current filter are drawn while taut segments are hidden to keep the canvas readable.
